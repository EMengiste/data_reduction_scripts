...
four=orange
five=red
# number of slip systems and set number ("1" "2" "3" "4" "5")
ss="2"
set="5"
...
neper -V "initial(type=ori):file(ini),Control(type=ori):file(${cu_cont}),One(type=ori):file(${cu1}),Two(type=ori):fi
le(${cu2}),Three(type=ori):file(${cu3}),Four(type=ori):file(${cu4}),Five(type=ori):file(${cu5})" \
     -space ipf                                     \
