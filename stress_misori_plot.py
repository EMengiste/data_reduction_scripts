...
simulations.sort()
sim_iso = fepx_sim("Cube.sim",path=iso_home+"isotropic/Cube.sim")
step= "28"
means = []
stds = []
set_start = 0
...
base = 90
for i in range(0,1,base):
       set_of_sims = [m for m in range(i,i+base,1)]
       sims = np.array([set_of_sims,np.tile(grain_start,(base)),np.tile(grain_end,(base)),np.tile(num__base,(base))]).T
...
df[1:].to_csv("calculation_stress_misori.csv")
